...
const PleuralEffusion = () => {
  ...
  const c = (section: string, key: string, fb: string) => getContent('pleural', section, key, fb);
  ...
  return (
    <Layout>
      <PageHero title={c('hero', 'title', 'Pleural Effusion Analysis')} subtitle={c('hero', 'subtitle', 'Comprehensive body fluid cytology for accurate diagnosis')} breadcrumb={[{ label: 'Applications', path: '/applications' }, { label: 'Pleural Effusion', path: '/applications/pleural-effusion' }]} />
      ...
      <motion.section className="py-16 lg:py-20" initial="hidden" whileInView="visible" viewport={viewportOnce} variants={sectionVariants}>
        <div className="container mx-auto px-6 lg:px-16 xl:px-24">
          ...
          {(() => {
            const pleuralFaqs = [
              { question: c('faq', 'q1', 'Which species are supported for fluid analysis?'), answer: c('faq', 'a1', 'Validated for dogs and cats.') },
              { question: c('faq', 'q2', 'What parameters can fluid analysis detect?'), answer: c('faq', 'a2', '19 parameters, including:\n\nNucleated cells: Inflammatory cells, granulocytes, lymphocytes, macrophages, neutrophils, degenerative neutrophils, phagocytic cells, mesothelial cells, and unclassified nucleated cells\n\nRBC-related: RBC, PCV\n\nMicroorganisms: Cocci, rods') },
              { question: c('faq', 'q3', 'What are the limitations for certain effusion samples?'), answer: c('faq', 'a3', 'Neoplastic effusions: AI may flag suspicious cells but cannot confirm tumor cells—manual review is required.\n\nSpecial effusions (e.g., biliary ascites or urinary ascites): Interpretation should be combined with biochemical testing.') },
            ];
