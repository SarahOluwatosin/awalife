...
const featureImages = [
  { image: images.speciesCanineFeline, alt: 'Veterinary species coverage' },
  { image: images.digitalMicroscope,   alt: 'Microscopy imaging for morphology' },
  { image: images.awalifeAnalyzerHero, alt: 'Awalife analyzer in clinical workflow' },
  { image: images.dm03Medtech,         alt: 'Support and training for clinics' },
];

const WhyUsSection = () => {
  const { getContent } = usePageContent();
  const c = (key: string, fb: string) => getContent('home', 'why_us', key, fb);

  const features = [
    {
      title: c('card_1_title', 'One platform that expands from multi-species analysis to multi-parameter evidence'),
      desc:  c('card_1_desc',  'It supports companion animals and small mammals, as well as large animals and a broad variety of exotic pets, making it suitable for a wide range of veterinary scenarios. In addition, it is capable of automatically analyzing blood, feces, urine, and pleural effusion samples, helping deliver efficient, accurate, and reliable results for daily clinical use.'),
    },
    {
      title: c('card_2_title', 'True-to-life morphology you can trust, capturing diagnostic details with clarity'),
      desc:  c('card_2_desc',  'With 40x optics, a 6.5 MP imaging system, and a wide field of view, Awalife preserves fine cellular detail while 1,000+ fields of view help ensure representative coverage. Liquid-based staining supports cell integrity for confident review and reporting.'),
    },
    {
      title: c('card_3_title', 'Morphology-first AI validated by real-world usage and rapid iteration'),
      desc:  c('card_3_desc',  'It is validated through real-world usage and rapid iteration, with over 15 million images used for AI training and more than 2.4 million diagnostic reports generated. Backed by 100+ innovations and patent updates, it continues to improve in performance and reliability with global field feedback.'),
    },
    {
      title: c('card_4_title', 'Support that scales: from day-to-day help to expert clinical guidance'),
      desc:  c('card_4_desc',  'Awalife offers responsive online support to keep workflows running smoothly, plus access to a clinical expert network for interpretive assistance. For distributors and partners, we provide a dedicated 1-on-1 support channel to streamline onboarding, technical escalation, and ongoing enablement.'),
    },
  ];
    ...
    <div className="container mx-auto px-6 lg:px-16 xl:px-24 relative z-10">
      ...
      <ScrollReveal className="max-w-4xl mx-auto text-center mb-16">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-foreground leading-tight">
          {c('title', 'An Expandable AI Morphology Platform')} <span className="gradient-text">{c('title_highlight', 'Grows with New Samples')}</span> {c('title_suffix', 'and Applications')}
        </h2>
      </ScrollReveal>
